...
const logger = new Logger('auth-service-multi-region');

/**
 * Initialize multi-region deployment for auth service
 */
export async function initializeMultiRegionDeployment(): Promise<void> {
  try {
    await initializeMultiRegion();
    logger.info('Multi-region deployment initialized for auth service');
  } catch (error) {
