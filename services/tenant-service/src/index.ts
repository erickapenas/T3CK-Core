...
if (!redisEnabled && !process.env.RATE_LIMIT_STORE) {
...
const dbEnabled =
  ...
  (process.env.DATABASE_ENABLED === 'true' || process.env.NODE_ENV === 'production');

if (!dbEnabled) {
  dbAvailable = false;
  logger.info('Database disabled for local/dev execution (using in-memory tenant storage)');
}
