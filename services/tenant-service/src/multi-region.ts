...
const logger = new Logger('tenant-service-multi-region');

/**
 * Initialize multi-region deployment for tenant service
 */
export async function initializeMultiRegionDeployment(): Promise<void> {
  try {
    await initializeMultiRegion();
    logger.info('Multi-region deployment initialized for tenant service');
  } catch (error) {
